<script lang="ts">
            ...
            try {
                ...
                // Gérer les messages Flipper (pour le bonus)
                    ...
                    flipperEvents.update(events => {
                        const newEvent = { deviceId, eventType, ...data, receivedAt: new Date() };
    ...
    function getDeviceStatus(lastUpdate: Date | undefined): 'online' | 'offline' | 'unknown' {
    ...
    $: {
        ...
        $telemetryData.forEach((data, deviceId) => {
            ...
            if (!filterLocation || filterLocation === 'all' || locationName.toLowerCase().includes(filterLocation.toLowerCase())) {
    ...
    $: globalAverage = (() => {
        ...
        $telemetryData.forEach((data, deviceId) => {
            if (getDeviceStatus(data.lastUpdate) === 'online') {
                // CORRECTION ICI : Utiliser tempC et humPct pour les moyennes
...
<style lang="postcss">
    ...
    .device-card {
        ...
        transition: transform 0.3s ease-in-out, border-left 0.3s ease-in-out;
        display: flex;
        flex-direction: column;
...
<div class="dashboard-container">
    ...
    <div class="average-card">
        <div class="average-item">
            ...
            <span class="average-value">{globalAverage.avgTemp} <span class="unit">°C</span></span>
        </div>
        <div class="average-item">
            <h3>Humidité Moyenne</h3>
            <span class="average-value">{globalAverage.avgHum} <span class="unit">%</span></span>
        </div>
        <div class="average-item">
            <h3>Appareils Actifs</h3>
            <span class="average-value">{globalAverage.onlineCount} / {$telemetryData.size}</span>
        </div>
    </div>
        ...
        {#each Array.from(filteredDevices.entries()) as [deviceId, data] (deviceId)}
            <div class="device-card {getDeviceStatus(data.lastUpdate)}">
                ...
                <div class="card-content">
                    <p><strong>Température:</strong> <span class="value">{data.tempC?.toFixed(1) ?? 'N/A'} °C</span></p>
                    <p><strong>Humidité:</strong> <span class="value">{data.humPct?.toFixed(1) ?? 'N/A'} %</span></p>
                    <p><strong>Batterie:</strong> <span class="value">{data.batteryPct ?? 'N/A'} %</span></p>
                </div>
            ...
            <div class="flipper-event-list">
                {#each $flipperEvents as event (event.receivedAt.getTime() + event.deviceId + event.eventType)}
                    <div class="flipper-event-item">
                        ...
                        <span class="payload">{JSON.stringify(event, ['button', 'action', 'card_uid', 'card_type', 'ax', 'ay', 'az', 'gx', 'gy', 'gz'])}</span>
